...
def convert_timestamps_to_iso8601(data: Union[Dict[str, Any], List[Dict[str, Any]]], 
                                  timezone_str: Optional[str] = None) -> Union[Dict[str, Any], List[Dict[str, Any]]]:
    """
    Convert Unix timestamp values to ISO8601 format for keys containing "timestamp".
    
    This function recursively processes dictionaries and lists, finding keys that contain
    the word "timestamp" and converting their Unix timestamp values to ISO8601 strings.
    If a timezone is provided, it also creates additional keys with timezone-aware timestamps.
    
    Parameters
    ----------
    data : dict or list of dict
        The input data structure containing timestamp fields to convert
    timezone_str : str, optional
        Timezone string (e.g., 'US/Eastern', 'UTC', 'America/New_York')
        If provided, creates additional keys with timezone-aware timestamps
        
    Returns
    -------
    dict or list of dict
        Data structure with converted timestamps
        
    Examples
    --------
    >>> data = {"timestamp": 1640995200, "values": [{"event_timestamp": 1640995260}]}
    >>> convert_timestamps_to_iso8601(data)
    {'timestamp': '2022-01-01T00:00:00Z', 'timestamp_iso8601': '2022-01-01T00:00:00Z', 
     'values': [{'event_timestamp': '2022-01-01T00:01:00Z', 'event_timestamp_iso8601': '2022-01-01T00:01:00Z'}]}
    
    >>> convert_timestamps_to_iso8601(data, timezone_str='US/Eastern')
    {'timestamp': '2022-01-01T00:00:00Z', 'timestamp_iso8601': '2022-01-01T00:00:00Z',
     'timestamp_timezone': '2021-12-31T19:00:00-05:00',
     'values': [{'event_timestamp': '2022-01-01T00:01:00Z', 'event_timestamp_iso8601': '2022-01-01T00:01:00Z',
                 'event_timestamp_timezone': '2021-12-31T19:01:00-05:00'}]}
    """
    def _convert_unix_to_iso8601(unix_timestamp: Union[int, float]) -> str:
    ...
    def _convert_unix_to_timezone(unix_timestamp: Union[int, float], tz_str: str) -> str:
        """Convert Unix timestamp to timezone-aware ISO8601 string."""
        try:
        ...
        except (ValueError, TypeError, OSError, pytz.exceptions.UnknownTimeZoneError):
            # Return original value as string if conversion fails
            return str(unix_timestamp)
    
    def _process_dict(obj: Dict[str, Any]) -> Dict[str, Any]:
        """Process a dictionary, converting timestamp fields."""
        result = {}
        ...
        for key, value in obj.items():
            ...
            elif "timestamp" in key.lower() and isinstance(value, (int, float)):
                # Convert the timestamp
                iso8601_value = _convert_unix_to_iso8601(value)
                result[key] = iso8601_value
                
                # Add ISO8601 key if it doesn't already exist
                iso8601_key = f"{key}_iso8601"
                if iso8601_key not in obj:
                    result[iso8601_key] = iso8601_value
                
                # Add timezone-aware timestamp if timezone provided
                if timezone_str:
                    timezone_key = f"{key}_timezone"
                    if timezone_key not in obj:
                        result[timezone_key] = _convert_unix_to_timezone(value, timezone_str)
            else:
                result[key] = value
                
        return result
    
    def _process_list(obj: List[Any]) -> List[Any]:
        """Process a list, converting timestamp fields in any dictionaries."""
        result = []
        for item in obj:
